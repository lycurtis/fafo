// PART 1: C++ implementation no use of ROS nodes
#include <thread>
#include <string>
#include <iostream>
#include <random>
...
void simTemp(RobotState *robot){
  ...
  while(1){
    std::this_thread::sleep_for(std::chrono::milliseconds(simTempPeriod)); // period
...
void simMotor(RobotState *robot){
  while(1){
    std::this_thread::sleep_for(std::chrono::milliseconds(simMotorPeriod)); // period
...
void simStatus(RobotState *robot){
  while(1){
    std::this_thread::sleep_for(std::chrono::milliseconds(simStatusPeriod)); // period
